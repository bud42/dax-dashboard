...
def update_reports(refresh=False):
    results = []
    ...
    for p in params_list:
        ...
        logger.info('updating project {}'.format(project_name))

        # Get our parameters
        logger.info('loading parameters:{}'.format(project_name))
        params = load_project_params(p)

        if 'reports' in params and params['reports'] == 'disabled':
        ...
        if os.path.isfile(filename):
            logger.info('file exists, will not overwrite:{}'.format(filename))
            return False

        logger.info('making report:{}'.format(filename))
...
def update_all(
    ...
    if was_triggered(ctx, 'button-reports-refresh'):
        ...
        logging.info('refresh:clicks={}'.format(n_clicks))
        refresh = True
